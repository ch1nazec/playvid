...
def generate_tag_slug(sender, instance, **kwargs):
    if instance.name_tag in Tag.objects.values_list('name_tag'):
        return
    if not instance.name_tag or not instance.name_tag.strip():
        # Если name_tag пустое, генерируем случайный slug
        import uuid
        instance.slug = f'untagged-{uuid.uuid4().hex[:8]}'
        return
    
    # Генерируем slug из name_tag
    base_slug = slugify(instance.name_tag)
    
    # Если slug пустой после slugify (например, только спецсимволы)
    if not base_slug:
        import uuid
